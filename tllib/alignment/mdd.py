...
class ImageClassifier(GeneralModule):
    ...
    def __init__(self, backbone: nn.Module, num_classes: int,
        ...
        head = nn.Sequential(OrderedDict([
            ('linear_h', nn.Linear(bottleneck_dim, width)),
            ('relu_h', nn.ReLU()),
            ('drop_h',nn.Dropout(0.5)),
            ('linear2_h',nn.Linear(width, num_classes))
        ]))
        '''head = nn.Sequential(
        ...
        for dep in range(2):
            ...
            adv_head[dep * 3].weight.data.normal_(0, 0.01) #normal_ changed to uniform
